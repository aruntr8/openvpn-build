...
!define PRODUCT_PUBLISHER "CITSAT Technologies"
...
VIAddVersionKey "ProductName" "Netkera-Installer"
VIAddVersionKey "Comments" ""
VIAddVersionKey "CompanyName" "NetKera Inc."
VIAddVersionKey "LegalTrademarks" "NetKera Inc."
VIAddVersionKey "LegalCopyright" "NetKera Inc."
VIAddVersionKey "FileDescription" "NetKera-Installer"
...
Section -pre
	...
	guiEndYes:
		DetailPrint "Closing ExampleVPN-GUI..."
		; user wants to close GUI as part of install/upgrade
		FindWindow $0 "ExampleVPN"
	...
	guiNotRunning:
		...
		${If} $R0 == 0
			MessageBox MB_OK|MB_ICONEXCLAMATION "The installation cannot continue as ExampleVPN is currently running. Please close all ExampleVPN instances and re-run the installer."
			Call RestoreServiceState
			Quit
		${EndIf}

		; ExampleVPN.exe + GUI not running/closed successfully, carry on with install/upgrade
...
Section /o "${PACKAGE_NAME} GUI" SecOpenVPNGUI
	...
	WriteRegStr HKLM "Software\Microsoft\Active Setup\Installed Components\${PACKAGE_NAME}_UserSetup" "" "ExampleVPN Setup"
	...
	${If} ${SectionIsSelected} ${SecLaunchGUIOnLogon0}
		WriteRegStr HKLM "Software\Microsoft\Active Setup\Installed Components\${PACKAGE_NAME}_UserSetup" "StubPath" "reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Run /v ExampleVPN /t REG_SZ /d $\"$INSTDIR\bin\openvpn-gui.exe$\" /f"
	${Else}
		WriteRegStr HKLM "Software\Microsoft\Active Setup\Installed Components\${PACKAGE_NAME}_UserSetup" "StubPath" "reg delete HKCU\Software\Microsoft\Windows\CurrentVersion\Run /v ExampleVPN-GUI /f"
	${EndIf}
SectionEnd
...
${IfNot} ${AtLeastWinVista}
	...
	MessageBox MB_YESNO|MB_ICONEXCLAMATION "This package does not work on your operating system.  The last version of ExampleVPN supported on your OS is 2.3. Shall I open a web browser for you to download it?" /SD IDNO IDYES DownloadForWinXP
	Quit
